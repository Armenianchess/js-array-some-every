...
function hasAZero(num) {
    return num.toString().split('').some(function (values) {
        return values === '0'
    })
}
...
Write a function called hasOnlyOddNumbers which accepts
 an array and returns true if every single number in the
  array is odd. If any of the values in the array are not 
  odd, the function should return false. 
...
of objects and a key, and returns true if every single object
 in the array contains that key. Otherwise it should return
  false.
...
single object in the array contains that value for the
